...
(function () {
  ...
  function getSummaries(f,t,chs){
    ...
    if(chs.native&&CH.native&&CH.native.enabled){const ns=D.summary&&D.summary.find(function(r){return r.channel==='Native'||r.channel===CH.native.label;});const pa=D.pacing&&D.pacing.find(function(r){return r.channel==='Native'||r.channel==='native';});const nImp=ns?ns.imp:(D.native_variations?D.native_variations.reduce(function(s,r){return s+r.imp;},0):0);const nClk=ns?ns.clicks:(D.native_variations?D.native_variations.reduce(function(s,r){return s+r.clicks;},0):0);const nSp=ns?ns.spend:(pa?pa.spent:0);if(nSp||nImp)out.push({name:CH.native.label,color:CH.native.color,budget:CH.native.budget||0,imp:nImp,clicks:nClk,cost:nSp,conv:0,ctr:nImp?nClk/nImp*100:0,cpm:nImp?nSp/nImp*1000:0});}
    return out;
  }
